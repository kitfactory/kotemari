...
*   **チェックリスト:**
    *   `[ ]` エラーハンドリングの見直しとカスタム例外 (`KotemariError` など) の定義・適用。
    *   `[ ]` **Controller:** `ApiController` の検討と実装 (FastAPIなどを使用？ UC-10)。(オプション)
    *   `[ ]` ドキュメント整備 (`README.md`, `README_ja.md` 更新、利用ガイド、APIリファレンスなど)。
    *   `[ ]` コード全体のレビューとリファクタリング。
