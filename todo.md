...
### Step 12: 差分更新における依存関係の波及処理 (11-2-3 詳細化)

*   **ゴール:** ファイル変更時に、そのファイルに依存している他のファイルの情報を自動的に更新できるようにする（依存関係の波及）。
*   **課題:** どのファイルがどのファイルに依存しているか（依存関係グラフ）を効率的に管理し、変更の影響範囲を特定する必要がある。
*   **実装方針:**
    *   **逆依存インデックスの構築:** メモリキャッシュ内に、ファイルパスをキーとし、そのファイルに依存しているファイルのパスリストを値とする辞書（逆依存インデックス）を保持する。例: `{'src/utils.py': ['src/main.py', 'src/api.py']}`。
    *   **インデックスの更新:** ファイルの依存関係が変更された場合（`ProjectAnalyzer` や `AstParser` が実行された結果）、この逆依存インデックスも更新する。
    *   **差分更新時の波及処理:**
        *   ファイルAが変更された場合、まずファイルA自体の `FileInfo` を更新する（Step 11-2-2）。
        *   次に、逆依存インデックスを参照し、ファイルAに依存しているファイル（例: ファイルB、ファイルC）を特定する。
        *   特定されたファイルB、ファイルCについて、それぞれの依存関係情報 (`FileInfo.dependencies`) を再計算（または無効化して再分析を促す）する。単純な更新か、再分析トリガーかはパフォーマンスと複雑性のトレードオフで決定。
*   **チェックリスト:**
    *   `[ ]` 12-1: 逆依存インデックス (`_reverse_dependency_index` など) を `Kotemari` クラスまたは適切な内部クラスに実装。
    *   `[ ]` 12-2: プロジェクト全体の分析 (`_analyze_project_internal`) 時に、逆依存インデックスを構築または更新する処理を追加。
    *   `[ ]` 12-3: ファイルの差分更新 (`_process_event` 内の `modified` イベント処理) 時、逆依存インデックスを利用して影響を受けるファイルを特定し、それらの依存関係情報を更新するロジックを実装。
    *   `[ ]` 12-4: 依存関係の波及が正しく行われることを確認するユニットテスト/結合テストを作成・実行。
